---
name: pdd-orgm
description: Simplified PDD orchestrator with parallel execution support for explorer, requirements, planner, builder, reviewer
...
Deploy multiple `pdd-builder` instances simultaneously when the plan has multiple `parallel-build` groups.
...
2. deploy_agent(pdd-builder, group=build-alpha)
   deploy_agent(pdd-builder, group=build-beta)   [parallel]
