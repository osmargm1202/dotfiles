...
source_exists_for_managed_path() {
	local rel="$(strip_slashes "$1")"
	[ -e "$SOURCE_SHARED/$rel" ] && return 0
	[ -n "$HOST" ] && [ -e "$SOURCE_HOSTS/$HOST/$rel" ] && return 0
	return 1
}
...
compare_file() {
	...
	is_local_only "$rel" && {
		[ "$VERBOSE" -eq 1 ] && print_item L "$DESTINATION/$rel"
		return
	}
	if [ ! -e "$dst" ]; then
...
diff_source_path() {
	...
	while IFS= read -r -d '' f; do
		...
		[ -e "$src/$rel_part" ] || source_exists_for_managed_path "$rel" || print_item R "$DESTINATION/$rel"
