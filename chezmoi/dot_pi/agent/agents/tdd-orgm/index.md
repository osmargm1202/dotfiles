...
- Use Engram memory workflow (`engram_mem_search` → `engram_mem_context` → `engram_mem_get_observation`) before non-trivial gate decisions; use `engram_mem_context` for broad cross-session TDD context.
